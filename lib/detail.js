...
const getDetail = async (url) => {
    ...
    const linkSub = await getSubLink(`${url}/play`)
    data['link_sub'] = linkSub;
...
const getSubLink = async (url) => {
    ...
    const html = await getHtml(url);
    const $ = cheerio.load(html);
    const text = $('input.movie-title').attr('value');
    return text;
}
...
// getSubLink('https://idxx1.cam/movie/logan-2017-subtitle-indonesia-5n0r/play').then(console.log)
